...
class Trainer:
	def __init__(self):
		...
		# Start Editing
		np.random.seed(25408)
		random.seed(25408)
		torch.manual_seed(25408)
		# End Editing
	...
	def test(self):
		...
		for batch in range(0, self.test_data.shape[0], self.batch_size):
			...
			# Find the loss
			loss = self.loss(prediction, batch_Y.long())
			# Find the number of correct predictions and update correct
			correct += (prediction.argmax(dim=-1)==batch_Y).float().sum()
	...
	def run_epoch(self):
		...
		self.train_data, self.train_labels = self.train_data[idx], self.train_labels[idx]
		...
		for batch in range(0, self.train_data.shape[0], self.batch_size):
			...
			# Find the loss
			tr_loss = self.loss(pred, batch_Y.long())
			# Backpropagation
			tr_loss.backward()
	...
	def predict(self, image):
		...
		prediction = self.model(image).argmax(dim=-1).float()
